...
const BrowseListings = () => {
    ...
    useEffect(() => {
        const fetchListings = async () => {
            try { 
                const res = await axios.get(`/consumer/food-not-contains-listings?_id=${userId}`);
    ...
    const handleBookNow = async (id, headLine, providerName) => {
        try {
            ...
            const res = await axios.put(`/consumer/bookListing/${id}`, { status: 'booked', consumerName, userId });
            if (res.status === 200) {
                ...
                {listings.map((listing, index) => (
                    <div key={index} className={Design.listingCard}>
                        ...
                        <div className={Design.button}>
                                {listing.status !== 'booked' && listing.status !== 'claimed' && (
                                    <button onClick={() => handleBookNow(listing._id, listing.headLine, listing.provider.name )}>Book Now</button>
                                )}
                                {listing.status === 'booked' && (
                                    <p><span className={Design.bookedStatus}> Booked : </span> {listing.consumerName} </p>
                                )}
                                {listing.status === 'claimed' && (
                                    <p><span style={{color:"red"}}> Claimed : </span> {listing.consumerName} </p>
                                )}
                            </div>
                            ...
                            <div className={Design.listingText}>
                                ...
                                <p><span>Contains :</span> {listing.foodContains.join(', ')}</p>
